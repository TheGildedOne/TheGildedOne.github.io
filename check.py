...
for source in sorted((Path(__file__).parent / "content" / "posts").glob("*.html")):
    ...
    # 8. in-body internal links.
    #
    # WRITING-LOOP.md has always said check.py enforced 2-4 of these. It did not:
    # section 5 above only verifies that links which exist resolve, so a post with
    # no links at all passed every check. The Apuleius post shipped on 2026-08-23
    # with zero, and nothing noticed.
    #
    # Only the body is counted. The related and prev/next blocks are generated by
    # build.py for every post, so counting rendered output would score a post with
    # no hand-written links as if it had several.
    #
    # Same date gate as the em-dash rule: earlier posts are grandfathered, and
    # tools/backfill_links.py is the way to raise those.
    body_only = source.read_text(encoding="utf-8").split("META-->", 1)[1]
    n_links = len(re.findall(r'href="/posts/', body_only))
    checked += 1
    if n_links < 2:
        errors.append(f"{source.name}: {n_links} in-body internal link(s), needs 2-4. "
                      f"Link an existing phrase to a related post; "
                      f"tools/backfill_links.py can suggest one.")
    elif n_links > 4:
        errors.append(f"{source.name}: {n_links} in-body internal links, house rule is 2-4.")
